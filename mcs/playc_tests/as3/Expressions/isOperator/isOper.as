...
package {
	...
	public class IsOperTests {

		public static function test():void {

			T.SECTION = "Expressions";       // provide a document reference (ie, Actionscript section)
			T.VERSION = "AS3";        // Version of ECMAScript or ActionScript 
			T.TITLE   = "is Operator";       // Provide ECMA section title or a description

			T.startTest();                // leave this alone
			...
			var typeArr:Array =		[String,Number,/*int*/System.Int32,/*uint*/System.Uint32,Boolean,Object,	Function,TestClassA,TestClassB,TestClassC,TestInterface,Date,Array];
			...
			var resultArr:Array  = new Array(typeArr.length);
			...
			for (var i:int = 0; i < valueArr.length; i++) {
				for (var j:int = 0; j < typeArrLength; j++) {
					T.AddTestCase(valueDescArr[i]+" is "+ typeDescArr[j],resultArr[i][j],(valueArr[i] is typeArr[j]));
				}
			}

			////////////////////////////////////////////////////////////////

			T.test();       // leave this alone.  this executes the test cases and
			              // displays results.
		}
	}
